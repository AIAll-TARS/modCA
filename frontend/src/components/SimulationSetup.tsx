...
export const SimulationSetup: React.FC<SimulationSetupProps> = ({ onStartSimulation }) => {
    ...
    // Show error state - with fallback to hardcoded defaults
    if (error || !constants) {
        console.log('SimulationSetup: Constants not available, using fallback defaults');
        
        // Fallback to hardcoded defaults if constants fail to load
        const fallbackConstants = {
            ...
            PREDATOR_BIRTH_PROBABILITY: 0.5,
            INITIAL_PREDATORS: 10,
            PREDATOR_STARVATION_STEPS: 10,
            PREY_HUNTED_PROBABILITY: 0.3,
            PREY_RANDOM_DEATH: 0.05,
            INITIAL_PREY: 50,
            PREY_BIRTH_PROBABILITY: 0.6,
            PREY_STARVATION_STEPS: 5,
            PREY_THREAT_RESPONSE: 0.3,
            INITIAL_SUBSTRATE_PROBABILITY: 0.3,
            SUBSTRATE_RANDOM_DEATH: 0.02,
            SUBSTRATE_CONSUMPTION_PROB: 0.9
        };
            ...
            <div className="space-y-6">
                <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4">
                    <div className="flex">
                        ...
                        <div className="ml-3">
                            <h3 className="text-sm font-medium text-yellow-800">Using Fallback Settings</h3>
                            <div className="mt-2 text-sm text-yellow-700">
                                <p>Could not load simulation constants from server. Using default values.</p>
                                <button
                                    onClick={() => {
                                        resetConstants();
                                        showToast('Retrying to load settings...', 'info');
                                    }}
                                    className="mt-2 bg-green-600 hover:bg-green-700 text-white font-medium py-1 px-3 rounded-md text-xs transition-colors"
                                >
                                    Retry
                                </button>
                            </div>
                ...
                {/* Render the form with fallback constants */}
                {renderForm(fallbackConstants)}
            </div>
        );
    }
